...
optresults_target=function(
  ...
  eval_g_ineq1 <- function( x ) {
    ...
    return(b=list( "constraints"= eval(parse(text=funcs$targetineq)),
                   "jacobian"= unlist(lapply(funcs$targetineqgrad,function(s) eval(s)  ))) 
    ) 
  } 
  ...
  for (i in 1:length(rn)){
    if(rn[i]%in%grplist){
      ...
      cst=value*get(paste0(gsub("grp_","",rn[1]),"pricepergrp"))
