roimeasure<-function(){
  ...
  salesdecomp2<-salesdecomposition(inputvar)
  lambda <- lambda1

  salesincrement <-data.frame(apply(salesdecomp2, 2,sum))
  salesincrement<-t(salesincrement);
  rownames(salesincrement)<-NULL
  
  
  startweek <- min(subset(stockdata, month == startMonth, select=week)) 
  
  inv_data_unstocked<- subset(unstockdata, month>=startMonth & month<=endMonth, select=c(medialist))   
  inv_data_stocked<- subset(stockdata, month>=startMonth & month<=endMonth, select=c(medialist))   
  
  
  stockedinvestment<-data.frame(lapply(inv_data_stocked,sum))
  
  inv_data_lastweek<- subset(stockdata, welibrary(linek == (startweek-1), select=c(medialist,"week"))) 
  
  
  
  investment_origin<-data.frame(lapply(inv_data_unstocked, sum))
  ...
  for(p in 1:length(vt)){ 
    ...
    x2<-as.double(inv_data_lastweek[[nameee]]*(1-lambda^(weekduration+1))/(1-lambda))
    ...
    if(nameee%in%grpgroup){
      if(nameee == "grp_all"){
        totalinv <-as.double(atlinvestment[2,p])*pricepergrp
      } else if (nameee == "grp_tv"){
        totalinv <-as.double(atlinvestment[2,p])*tvpricepergrp
      } else if (nameee == "grp_ca"){
        totalinv <-as.double(atlinvestment[2,p])*catvpricepergrp
      } else if (nameee == "grp_jp"){
        totalinv <-as.double(atlinvestment[2,p])*jptvpricepergrp
      }
